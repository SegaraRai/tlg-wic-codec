...
int SaveTLG5(tTJSBinaryStream* out, int width, int height, int colors, void* callbackdata, tTVPGraphicScanLineCallback scanlinecallback) {
  ...
  // header
  if (!out->WriteBuffer("TLG5.0\x00raw\x1a\x00", 11) || !out->WriteBuffer(&colors, 1) || !out->WriteInt32(width) || !out->WriteInt32(height) || !out->WriteInt32(BLOCK_HEIGHT)) {
    return TLG_ERROR;
  }
  ...
  try {
    ...
    for (int blk_y = 0; blk_y < height; blk_y += BLOCK_HEIGHT, block++) {
      ...
      for (int c = 0; c < colors; c++) {
        ...
        if (wrote < inp) {
          if (!out->WriteBuffer("\x00", 1) || !out->WriteInt32(wrote) || !out->WriteBuffer(cmpoutbuf[c], wrote)) {
            ret = TLG_ERROR;
            goto errend;
          }
          blocksize += wrote + 4 + 1;
          ...
          compressor->Restore();
          if (!out->WriteBuffer("\x01", 1) || !out->WriteInt32(inp) || !out->WriteBuffer(cmpinbuf[c], inp)) {
            ret = TLG_ERROR;
            goto errend;
          }
          blocksize += inp + 4 + 1;
