...
//-- must implement
tjs_uint64 tHandleStream::Seek(tjs_int64 offset, tjs_int whence) {
...
tjs_uint tHandleStream::Read(void* buffer, tjs_uint read_size) {
  if (handle != INVALID_HANDLE_VALUE) {
    DWORD readBytes;
...
tjs_uint tHandleStream::Write(const void* buffer, tjs_uint write_size) {
  if (handle != INVALID_HANDLE_VALUE) {
    DWORD writeBytes;
