...
bool tTJSBinaryStream::ReadBuffer(void* buffer, tjs_uint read_size) {
  return Read(buffer, read_size) == read_size;
}

bool tTJSBinaryStream::WriteBuffer(const void* buffer, tjs_uint write_size) {
  return Write(buffer, write_size) == write_size;
}

bool tTJSBinaryStream::ReadI64LE(tjs_uint64& value) {
...
bool tTJSBinaryStream::ReadI32LE(tjs_uint32& value) {
#if TJS_HOST_IS_BIG_ENDIAN
  tjs_uint8 buffer[4];
...
bool tTJSBinaryStream::ReadI16LE(tjs_uint16& value) {
#if TJS_HOST_IS_BIG_ENDIAN
  tjs_uint8 buffer[2];
...
bool tTJSBinaryStream::WriteInt32(long num) {
  unsigned char buf[4];
  buf[0] = num & 0xff;
  buf[1] = (num >> 8) & 0xff;
  buf[2] = (num >> 16) & 0xff;
...
void tTJSBinaryStream::CopyFrom(tTJSBinaryStream* stream, int pos) {
  char buf[BUFSIZE];
