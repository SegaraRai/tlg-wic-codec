...
char TVPTLG6GolombBitLengthTable[TVP_TLG6_GOLOMB_N_COUNT * 2 * 128][TVP_TLG6_GOLOMB_N_COUNT] = { { 0 } };
...
void TVPTLG6InitLeadingZeroTable(void) {
  ...
  for (i = 0; i < TVP_TLG6_LeadingZeroTable_SIZE; i++) {
    int cnt = 0;
    int j;
    for (j = 1; j != TVP_TLG6_LeadingZeroTable_SIZE && !(i & j); j <<= 1, cnt++)
      ;
    cnt++;
...
TVP_GL_FUNC_DECL(void, TVPTLG5ComposeColors3To4, (tjs_uint8 * outp, const tjs_uint8* upper, tjs_uint8* const* buf, tjs_int width)) {
  ...
  for (x = 0; x < width; x++) {
    ...
    c[0] += c[1];
    c[2] += c[1];
    *(tjs_uint32*)outp = ((((pc[0] += c[0]) + upper[0]) & 0xff)) + ((((pc[1] += c[1]) + upper[1]) & 0xff) << 8) + ((((pc[2] += c[2]) + upper[2]) & 0xff) << 16) + 0xff000000;
    outp += 4;
    upper += 4;
  }
}
...
TVP_GL_FUNC_DECL(void, TVPTLG5ComposeColors4To4, (tjs_uint8 * outp, const tjs_uint8* upper, tjs_uint8* const* buf, tjs_int width)) {
  ...
  for (x = 0; x < width; x++) {
    ...
    c[2] += c[1];
    *(tjs_uint32*)outp =
      ((((pc[0] += c[0]) + upper[0]) & 0xff)) + ((((pc[1] += c[1]) + upper[1]) & 0xff) << 8) + ((((pc[2] += c[2]) + upper[2]) & 0xff) << 16) + ((((pc[3] += c[3]) + upper[3]) & 0xff) << 24);
    outp += 4;
    upper += 4;
  }
}
...
#  define TVP_TLG6_FETCH_32BITS(addr) \
    ((tjs_uint32)TVP_TLG6_BYTEOF((addr), 0) + ((tjs_uint32)TVP_TLG6_BYTEOF((addr), 1) << 8) + ((tjs_uint32)TVP_TLG6_BYTEOF((addr), 2) << 16) + ((tjs_uint32)TVP_TLG6_BYTEOF((addr), 3) << 24))
#else
...
#define TVP_TLG6_DO_CHROMA_DECODE_PROTO2(B, G, R, A, POST_INCREMENT)                                  \
  do {                                                                                                \
    tjs_uint32 u = *prevline;                                                                         \
    p = avg(p, u, up, (0xff0000 & ((R) << 16)) + (0xff00 & ((G) << 8)) + (0xff & (B)) + ((A) << 24)); \
    up = u;                                                                                           \
    *curline = p;                                                                                     \
    curline++;                                                                                        \
    prevline++;                                                                                       \
