 ...
 */
int TVPSaveTLG(tTJSBinaryStream* dest, int type, int width, int height, int colors, void* callback, tTVPGraphicScanLineCallback scanlinecallback, const std::map<std::string, std::string>* tags) {
  ...
  // write tag chunk
  if (!dest->WriteInt32(s.length()) || !dest->WriteBuffer(s.c_str(), s.length())) {
    return TLG_ERROR;
  }
